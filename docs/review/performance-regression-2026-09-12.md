...
- **Device-only geometry failure:** the Android screenshot exposed a second cause of the missing playhead: the crossfade's active layer contained only absolutely positioned children and therefore had zero height. Explicit fill geometry now gives both crossfade layers the viewport bounds, restoring the played clip and playhead. The same mode repairs absolute mini-player borders while preserving natural sizing for controls. The interaction smoke checks actual playhead pixels and the darker played color.
...
`[WaveformTiming]` with `source: analysis` records the decoder's own elapsed time, including idle preparation; cache/native load timings remain separate. Android UI inspection uses pinned `uiautomator2` 3.7.0 with idle waiting disabled: continuous playback prevented the shell dump command from refreshing its XML. A software-rendered 1080p cold emulator run hit the existing setup timeout; the replay uses 540×1200 pixels at the same logical layout and records any visible hydration retry instead of hiding it.

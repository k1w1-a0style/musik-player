...
class AndroidSafContentStore(private val context: Context) : SafContentStore {
  ...
  override fun isWritable(uri: Uri): Boolean = try {
    val queryUri = when { DocumentsContract.isDocumentUri(context, uri) -> uri; DocumentsContract.isTreeUri(uri) -> DocumentsContract.buildDocumentUriUsingTree(uri, DocumentsContract.getTreeDocumentId(uri)); else -> return false }
  ...
  override fun size(uri: Uri): Long? = try {
  ...
  private fun isUriCoveredByPersistedTreePermission(permissionUri: Uri, targetUri: Uri): Boolean = try {
    ...
    val targetDocumentId = when { DocumentsContract.isDocumentUri(context, targetUri) -> DocumentsContract.getDocumentId(targetUri); DocumentsContract.isTreeUri(targetUri) -> DocumentsContract.getTreeDocumentId(targetUri); else -> return false }

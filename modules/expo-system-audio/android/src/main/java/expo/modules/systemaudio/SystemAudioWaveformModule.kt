...
class SystemAudioWaveformModule : Module() {
  ...
  private fun readDistributedSampleEnvelope(
    ...
    // allocate buffer once per call and reuse
  ...
  private fun normalize(peaks: DoubleArray): List<Double> {
    ...
    // Use a single consistent floor so the UI is predictable. Previously two different
    // floors (0.04 then max(0.08,...)) resulted in inconsistent behavior.
    val floor = 0.08
    return peaks.map { peak ->
      val normalized = (peak / maxPeak).coerceIn(floor, 1.0)
      normalized
    }
  }

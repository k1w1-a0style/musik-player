...
class AudioTagTransactionManagerTest {
  ...
  private fun prepared(state: TransactionState): File { val root=tmp(); val st=TransactionStorage(root); val d=st.createDir(); val o="old".toByteArray(); st.original(d).writeBytes(o); st.atomicWriteJournal(d, TransactionJournal(transactionId=d.name,targetUri=uri.toString(),state=state,createdAtEpochMs=1,updatedAtEpochMs=1,originalSizeBytes=o.size.toLong(),originalSha256Hex=sha(o))); return root }
  private fun recoverState(state: TransactionState) { val root=prepared(state); val r=manager(root,FakeStore("broken".toByteArray())).recoverPending(); assertFalse(r.success); assertTrue(r.recovered || r.errorCode == "RecoveryFailed") }
}

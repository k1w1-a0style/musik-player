...
import { createNativeHydrationExpectation, evaluateNativeHydrationFulfillment } from '../nativeHydrationPlanFulfillment';
import { createHydrationPlan } from '../musicHydrationPlan';
import type { NativeQueueReadback } from '../nativeQueueRecovery';

const songs: Song[] = [{ id: 's1', title: 'One', artist: 'A', uri: 'file:///1.mp3' }];
...
const readback = (queue: Song[], activeTrackId: string | null, activeIndex: number): NativeQueueReadback => ({
  queue, activeSong: activeIndex >= 0 ? queue[activeIndex] ?? null : null, activeTrackId, activeIndex,
  progressSeconds: 0, playbackState: 'paused',
});
...
test('exact active expectation requires matching id and index', () => {
  ...
  expect(evaluateNativeHydrationFulfillment(expectation, readback(songs, 's1', 0)).fulfilled).toBe(true);
  expect(evaluateNativeHydrationFulfillment(expectation, readback(songs, null, -1)).reason).toBe('active-id');
  expect(evaluateNativeHydrationFulfillment(expectation, { ...readback(songs, 's1', 0), activeIndex: 1 }).reason).toBe('active-index');
});
...
test('current-song-less no-op expectation leaves the active track unspecified', () => {
  ...
  const snapshot = { ...readback([], null, -1), nativeQueue: [], baseQueue: [], shuffleEnabled: false };

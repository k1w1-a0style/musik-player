export const libraryImportMessages = {
  ...
  permissionRequiredMessage: 'Ohne Zugriff können keine Songs importiert werden.',
  ...
  scanFailedMessage: 'In den Scan-Ordnern wurden keine importierbaren Songs gefunden. Einige Ordner/Dateien waren nicht lesbar.',
  partiallyImportedTitle: 'Teilweise importiert',
  partiallyImportedMessage: 'Einige Ordner/Dateien waren nicht lesbar. Importierbare Songs wurden trotzdem übernommen.',
};
...
export const tracksFoundStatus = (count: number): string =>
  `${count} Tracks gefunden. Bibliothek wird aktualisiert…`;
...
export const tracksSavingStatus = (count: number): string =>
  `${count} Tracks werden gespeichert…`;

export const metadataRefreshSummary = (updated: number, skipped: number, failed: number): string =>
  `${updated} Tracks aktualisiert. ${skipped} übersprungen. ${failed} fehlgeschlagen.`;

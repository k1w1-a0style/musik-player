...
const SENSITIVE_FIELD_PATTERN = /(?:uri|url|path|file(?:name)?|document(?:id)?|authority|authorization|auth|credential|token|secret|password|passphrase|keystore(?:[_-]?base64)?|(?:secret|service[_-]?role|admin|api|access|private)[_-]?key|title|artist|album|genre|comment|lyrics)$/i;

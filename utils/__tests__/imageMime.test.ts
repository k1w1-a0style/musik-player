...
describe('imageMime helpers', () => {
  ...
  test('requires every signature byte at its exact offset', () => {
    expect(detectImageMimeFromBytes(new Uint8Array([0xff, 0xd8, 0x00, 0xff]))).toBeUndefined();
    expect(detectImageMimeFromBytes(new Uint8Array([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x00]))).toBeUndefined();
    expect(detectImageMimeFromBytes(new Uint8Array([0x52, 0x49, 0x46, 0x46, 0x57, 0x45, 0x42, 0x50, 0x00, 0x00, 0x00, 0x00]))).toBeUndefined();
    expect(detectImageMimeFromBytes(new Uint8Array([0x00, 0x00, 0x00, 0x00, 0x52, 0x49, 0x46, 0x46, 0x57, 0x45, 0x42, 0x50]))).toBeUndefined();
  });
  ...
  test('returns undefined for unsupported or truncated image bytes', () => {
    ...
    expect(detectImageMimeFromBytes(new Uint8Array([0x52, 0x49, 0x46, 0x46, 0x00, 0x00, 0x00, 0x00, 0x57, 0x45, 0x42]))).toBeUndefined();
